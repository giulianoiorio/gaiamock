...
def get_5par_solution_and_sigma_5d_max(t_ast_yr, psi, plx_factor, ast_obs, ast_err):
    '''
    This function takes a set of astrometric data (t_ast_yr, psi, plx_factor, ast_obs, ast_err) and fits a 5-parameter solution. It inflates the uncertainties according to the goodness of fit and returns the 5-parameter UWE, best-fit parameters, and uncertainties. 
...
def check_7par(t_ast_yr, psi, plx_factor, ast_obs, ast_err):
    ...
    Lambda_pred = np.dot(M, mu)
    resids = ast_obs - Lambda_pred
    Nobs, nu = len(ast_obs), len(ast_obs) - 7
...
def check_9par(t_ast_yr, psi, plx_factor, ast_obs, ast_err):
    ...
    Nobs, nu = len(ast_obs), len(ast_obs) - 9 
    chi2_red = np.sum(resids**2/ast_err**2)/nu
...
def get_gost_one_position(ra, dec, data_release):
    '''
    this function finds the scan times and angles for a given sky position and data release by searching for the nearest position in a pre-downloaded set of 3072 sky positions (healpix level 16). For this to work, you need to have downloaded healpix_scans.zip and unzipped it into gaiamock/healpix_scans. 
...
def get_uncertainties_at_best_fit_binary_solution(t_ast_yr, psi, plx_factor, ast_obs, ast_err, p0, c_funcs, reject_outlier=False):
    ...
    if not np.sum(~np.isfinite(cov_x)):
        ...
        if reject_outlier:
            nu, Nobs = len(ast_obs)-12-1, len(ast_obs)-1
        else:
            nu, Nobs = len(ast_obs) - 12, len(ast_obs)
        chi2_red= np.sum(resid_func(p0)**2)/nu
        cc = np.sqrt(chi2_red/((1 - 2/(9*nu))**3 ))
...
def fit_5par_solution_only(t_ast_yr, psi, plx_factor, ast_obs, ast_err):
    '''
    ...
    ruwe, mu, sigma_mu, sigma5d_max = get_5par_solution_and_sigma_5d_max(t_ast_yr = t_ast_yr, psi = psi, plx_factor = plx_factor, ast_obs=ast_obs, ast_err=ast_err)
...
def fit_full_astrometric_cascade(t_ast_yr, psi, plx_factor, ast_obs, ast_err, c_funcs, verbose=False, show_residuals=False, ruwe_min = 1.4, skip_acceleration=False, reject_outlier=False, P_min = 10):
    ...
    errors, a0_mas, sigma_a0_mas, inc_deg = get_uncertainties_at_best_fit_binary_solution(t_ast_yr = t_ast_yr, psi = psi, plx_factor = plx_factor, ast_obs = ast_obs, ast_err = ast_err, p0 = p0, c_funcs = c_funcs, reject_outlier=reject_outlier)
    sig_ra,sig_dec,sig_parallax,sig_pmra,sig_pmdec,sig_period,sig_ecc,sig_phi_p,sig_A,sig_B, sig_F,sig_G = errors
    
    if reject_outlier:
        Nobs, nu = len(ast_obs)-1, len(ast_obs)-12-1
    else:
        Nobs, nu = len(ast_obs), len(ast_obs) - 12
    chi2_red = chi2/nu
